...
function parseQuery (query) {
  ...
  if (keys[0] === "$and" || keys[0] === "$or") {
    ...
    if (typeof query[keys[0]] === 'string') {
      ...
      if (query[keys[0]].order !== undefined) {
        operand.order = query[keys[0]].order;
      }
      ...
      if (query[keys[0]][innerKeys[0]].order !== undefined) {
        operand.order = query[keys[0]][innerKeys[0]].order;
      }
    }
  }
  ...
  if (tree.operand === "$and" || tree.operand === "$or") {
    ...
    index.searchIndexes(tree, function indexCallback (err, results, metrics) {
      if (err === "no indexes for query") {
        ...
        // or worst case, in the case of 'and' to create a readstream of only those
        // gets and deal with only those. http-db has a good example.
