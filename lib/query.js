...
function runQuery (tree, index, store, callback) {
  ...
  if (key === "$and" || key === "$or") {
    ...
    for (var i = 0; i < len; i++) {
      runQuery(tree[key].value[i], index, store, function (err, results, metrics) {
        ...
        if (done === len) {
          if (key === "$and") {
            results = and.apply(keys);
          } else {
            results = or.apply(keys);
          }
          ...
          callback(null, keys, compiledMetrics);
        }
      });
    ...
    index.searchIndexes(tree, function (err, results, metrics) {
      if (err === "no indexes for query") {
        ...
        stream.on('data', function (data) {
          var result = utils.compare(tree[key], data);
          if (result) {
